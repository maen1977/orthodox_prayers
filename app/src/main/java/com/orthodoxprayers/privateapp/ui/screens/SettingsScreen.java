...
public final class SettingsScreen extends BaseScreen {
    ...
    public View createView() {
        ...
        smaller.setContentDescription(local("تصغير حجم النص", "Decrease text size", "Μείωση μεγέθους κειμένου"));
        ...
        larger.setContentDescription(local("تكبير حجم النص", "Increase text size", "Αὔξηση μεγέθους κειμένου"));
        ...
        Button resetReading = ui.button(local("إعادة إعدادات القراءة للوضع الافتراضي", "Reset reading settings", "Ἐπαναφορὰ ρυθμίσεων ἀνάγνωσης"), false);
        resetReading.setOnClickListener(v -> new AlertDialog.Builder(host.activity())
                .setTitle(local("إعادة إعدادات القراءة؟", "Reset reading settings?", "Ἐπαναφορὰ ρυθμίσεων ἀνάγνωσης;"))
                .setMessage(local(
                        "سيعود حجم النص والخط والتباعد وثيم القارئ والسطوع والتمرير التلقائي إلى الوضع الافتراضي.",
                        "Text size, font, spacing, reader theme, brightness, and auto-scroll will return to their defaults.",
                        "Τὸ μέγεθος, ἡ γραμματοσειρά, τὸ διάστιχο, τὸ θέμα, ἡ φωτεινότητα καὶ ἡ αὐτόματη κύλιση θὰ ἐπανέλθουν στὶς προεπιλογές."
                ))
                .setPositiveButton(local("إعادة", "Reset", "Ἐπαναφορά"), (dialog, which) -> {
        ...
        Button diagnosticsToggle = ui.button(preferences.advancedDiagnosticsExpanded()
                ? local("إخفاء التفاصيل التقنية", "Hide technical details", "Ἀπόκρυψη τεχνικῶν λεπτομερειῶν")
                : local("عرض التفاصيل التقنية", "Show technical details", "Προβολὴ τεχνικῶν λεπτομερειῶν"),
        ...
        if (preferences.advancedDiagnosticsExpanded()) {
            TextView status = centered(local("آخر محاولة تحديث: ", "Last update attempt: ", "Τελευταία προσπάθεια: ") + lastAttempt
                    + "\n" + local("رمز تشخيص التحديث: ", "Update diagnostic code: ", "Κωδικὸς διαγνώσεως: ") + LocalePolicy.isolateTechnical(data.refreshDiagnosticCode())
    ...
    private void addReminder(LinearLayout root, String kind, String label, int fallbackMinute) {
        ...
        time.setContentDescription(local("تغيير وقت ", "Change time for ", "Ἀλλαγὴ ὥρας γιὰ ") + label);
    ...
    private String fontFamilyLabel() {
        if ("serif".equals(preferences.fontFamily())) return local("كتاب", "Serif", "Μὲ πατούρες");
        if ("monospace".equals(preferences.fontFamily())) return local("ثابت العرض", "Monospace", "Σταθεροῦ πλάτους");
        return local("بسيط", "Sans", "Χωρὶς πατούρες");
    }
    ...
    private void showTimePicker(int currentMinute, MinuteSelection callback) {
        ...
        new AlertDialog.Builder(host.activity())
                .setTitle(local("اختيار الوقت", "Choose time", "Ἐπιλογὴ ὥρας"))
                .setView(wrapper)
