...
# Romans 14:24-26 are numbered Romans 16:25-27 in the canonical lectionary
# tradition used by this project.  Both the World English Bible and the 1904
# Patriarchal Greek source archive bundled by eBible place this doxology at
# Romans 14:24-26.  Never rewrite wording here; only expose an additional
# canonical coordinate while preserving the source coordinate in metadata.
_ROMANS_DOXOLOGY_RELOCATION = {
    ("ROM", 16, 25): ("ROM", 14, 24),
    ("ROM", 16, 26): ("ROM", 14, 25),
    ("ROM", 16, 27): ("ROM", 14, 26),
}
SOURCE_VERSE_RELOCATIONS: dict[str, dict[tuple[str, int, int], tuple[str, int, int]]] = {
    "en": dict(_ROMANS_DOXOLOGY_RELOCATION),
    "el": dict(_ROMANS_DOXOLOGY_RELOCATION),
}
    ...
    for attempt in range(attempts):
        try:
            request = urllib.request.Request(
                url,
                headers={
                    "User-Agent": "OrthodoxPrayersDailyUpdater/5.6.5 (+https://github.com/maen1977/orthodox_prayers)",
