...
for token in ['versionCode = 50502','versionName = "5.5.2"','prepareChurchServiceCorpus','generated/churchServiceAssets']:
...
# 5.5.2 rights/source hardening: protected GOARCH pages are not scraped into the APK.
for lang in ('en','el'):
...
for token in ['public_domain_plain_text','cc_by_pdf_text','CHURCH_SERVICE_RIGHTS_LINK_ONLY','rights_basis','permission_confirmed']:
...
if errors:
    ...
    sys.exit(1)
print('FULL_CHURCH_SERVICES_OK version=5.5.2 policy=rights_aware_no_translation generated=' + (','.join(materialized) if materialized else 'not_materialized_in_source_checkout'))
