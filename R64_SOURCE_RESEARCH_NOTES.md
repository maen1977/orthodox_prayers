...
## Final native-commemoration corpus checkpoint (2026-08-23)

The earlier 148-slot Arabic paragraph above is a historical checkpoint, not the final state. After reviewing the remaining Arabic contact sheets and individual source-page crops, the repository now promotes **361 of 366 Arabic old-calendar slots** from the official Jerusalem Arabic 2024 PDF. The five Arabic slots still marked `REQUIRES_VISUAL_REVIEW` are `02-16`, `04-23`, `06-18`, `07-31`, and `11-13`; they remain unfilled because a proper name or a clipped row could not be established without inference. The Arabic packet is split into the base packet plus additional audit packets through `arabic_visual_review_batch_6.json`, and each promoted lane retains the Arabic source URL, source year, jurisdiction, and page evidence.

The Greek 2025 calendar was then reviewed directly at printed pages 62 and 66. The two former look-alike glyph flags were resolved as Greek characters: `Ἀρχαγγέλου` at old slot `03-26` and `Κεφαλῆς` at old slot `05-25`. Greek coverage is now **311 of 366 local fixed-slot records**, with no Greek visual-review rows remaining; the 311 includes the separately documented 02-29 leap-day record from the Greek 2024 source. The Greek text was not translated or reconstructed from Arabic or English.

The English corpus remains a complete **366-slot native-English comparative corpus** from Holy Trinity/Jordanville. It is licensed and displayed through the English sidecar, but its jurisdiction is explicitly `comparative_not_jerusalem_jordan`; it is not counted as local Jerusalem/Jordan evidence. Consequently, `strict_named_local_gate=false` and `strict_local_three_language_slots=0` remain intentional and truthful. This checkpoint does not claim that the local Jerusalem/Jordan English corpus exists, nor does it authorize production release or ecclesiastical approval.
