...
def test_settings_hub_uses_action_cards_and_keeps_about_plain() -> None:
    ...
    assert settings.count("addSettingsAction(page,") == 4
    assert "ic_action_language" in settings
    assert "ic_action_brightness" in settings
...
def test_each_settings_card_opens_its_own_back_navigable_detail_page() -> None:
    ...
    for section in ("language", "font_size", "calendar_reminders", "update_data"):
        assert f'"{section}"' in settings
    assert "if (SECTION_LANGUAGE.equals(section)) return createLanguageSettingsView();" in settings
    assert "if (SECTION_FONT_SIZE.equals(section)) return createFontSettingsView();" in settings
