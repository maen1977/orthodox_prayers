...
def package(start: date, language: str | None = None, *, days: int = 9) -> dict:
    ...
    value["rolling_week"] = {
        "schema_version": 1,
        "policy": MODULE.POLICY if days == 9 else "TODAY_PLUS_SEVEN_DAYS",
...
def test_current_nine_day_contract_is_accepted():
    assert MODULE.published_payload_errors(package(date(2026, 7, 30)), "2026-07-30") == []


def test_legacy_eight_day_contract_is_rejected():
    errors = MODULE.published_payload_errors(
        package(date(2026, 7, 30), days=8),
        "2026-07-30",
    )
    assert any("nine-day policy" in error for error in errors)
    assert any("day_count must be 9" in error for error in errors)
    assert any("eight future days" in error for error in errors)
...
def test_root_contract_checks_canonical_and_language_lanes():
    with tempfile.TemporaryDirectory() as directory:
        ...
        calendar.write_text(json.dumps(package(start)), encoding="utf-8")
        for language in ("ar", "en", "el"):
            ...
            content = json.dumps(package(start, language), ensure_ascii=False)
