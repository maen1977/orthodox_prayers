...
def test_current_moving_window_contract_is_accepted():
    assert MODULE.published_payload_errors(package(date(2026, 7, 30), days=21), "2026-07-30") == []
...
def test_too_short_window_contract_is_rejected():
    ...
    assert any("unsupported" in error or "between 9 and 42" in error for error in errors)
...
def test_root_contract_checks_canonical_and_language_lanes():
    with tempfile.TemporaryDirectory() as directory:
        ...
        calendar.write_text(json.dumps(package(start, days=21)), encoding="utf-8")
        for language in ("ar", "en", "el"):
            ...
            content = json.dumps(package(start, language, days=21), ensure_ascii=False)
