...
- إضافة روابط مباشرة للبروسكوميدي والسَحَر والساعات وصلوات ما قبل/بعد المناولة.
- بدء إعادة بناء بيانات اليوم تلقائيًا عند تشغيل التطبيق إذا كانت اللقطة المضمّنة قديمة.
- تضمين نصوص الرسالة والإنجيل لكل مراجع الرزنامة 2026–2050 في العربية والإنجليزية واليونانية دون اتصال بالإنترنت.
- إدخال قراءات اليوم الموثقة في مواضع الرسالة والإنجيل داخل القداس المعيّن.

## 5.6.2 — مقاومة انقطاع مصادر الخدمات الكنسية أثناء البناء

- لا يعود انقطاع شبكة `orthodoxjordan.org` في GitHub Actions سببًا لإيقاف APK بالكامل؛ الخدمات العربية المطلوبة تتحول مؤقتًا إلى رابط المصدر العربي الرسمي فقط، بدون نص من لغة أخرى وبدون ادعاء أن الخدمة الكاملة مضمّنة.
- إضافة `fallbacks` صريحة إلى حزمة الخدمات المولّدة مع `full_service=false` و`machine_translation_used=false` و`cross_language_fallback=false`.
- إضافة حد أقصى منطقي لحجم استخراج الخدمة الواحدة (`80,000` حرف افتراضيًا) لمنع اعتماد استخراج خاطئ مثل خدمة الخطوبة الإنجليزية التي وصلت إلى مئات آلاف الأحرف.
- بقاء المصدر الرسمي هو المحاولة الأولى دائمًا؛ عند عودة الشبكة يعود الاستيراد الكامل تلقائيًا إذا اجتاز فحوص الطول واللغة والحقوق.
- إضافة اختبارات تحاكي `Errno 101 Network is unreachable` وتثبت أن المولّد ينتهي بـ exit code 0 مع fallback آمن.

## 5.6.1 — إصلاح ظهور قسم القداس

- يبقى تبويب **القداس** ظاهرًا دائمًا كوجهة رئيسية مستقلة عن حالة نص خدمة اليوم.
- أضيفت شاشة `LiturgyHubScreen` التي تعرض نوع قداس اليوم من الرزنامة وحالة توفر النص، ثم تفتح القارئ فقط عندما تكون الطبعة المعيّنة كاملة وقابلة للعرض.
- بطاقة القداس في الرئيسية تمر عبر شاشة اليوم الذكية بدل فتح القارئ مباشرة.
- أيام باسيليوس/السابق تقديسه/عدم وجود قداس لا تُخفي القسم ولا تستبدل طقسًا بآخر.
- أزرار القداس غير المكتمل في تفاصيل الأيام تبقى ظاهرة لكن غير قابلة لفتح نص جزئي كمراسم كاملة.

## 5.6.0 — المحرك الذكي للقداس الإلهي

- رفع الإصدار إلى 5.6.0 (`50600`).
- فصل القداس الإلهي الصارم من البركة الافتتاحية حتى الصرف عن السَحَر والساعات والبروسكوميدي وصلوات المناولة الشخصية والشكر.
- إزالة إنجيل السَحَر من dynamic slots الخاصة بالقداس؛ يبقى مرجعًا مستقلًا للسَحَر.
- إضافة `liturgy_day_plan` الذي يثبت نوع القداس، شكل الخدمة، قاعدة الاختيار، قراءتي الرسالة والإنجيل، وسياسة عدم إدخال أي مادة غير معيّنة.
- منع `LocalDailyContentEngine` من ربط قالب الذهبي الفم مسبقًا عند اختيار باسيليوس/السابق تقديسه/يوم بلا قداس.
- تصحيح تسمية `lenten_vespers_with_presanctified` في المحرك المحلي.
- تدقيق الصدق النصي: الذهبي الفم قابل للنشر في AR/EN/EL؛ باسيليوس يبقى محجوبًا بسبب AR/EL OCR؛ السابق تقديسه يبقى محجوبًا لأن AR الحالي شرح وليس الخدمة الكاملة.
- إضافة `validate_smart_liturgy_core.py` إلى بوابة الجودة.

## 5.5.2 — مصادر خدمات كنسية مفتوحة وحقوق نشر دقيقة

- رفع الإصدار إلى 5.5.2 (50502).
- إزالة صفحات GOARCH المحمية من مسار بناء النصوص الكنسية؛ لم يعد نجاح CI يعتمد على تجاوز HTTP 403 أو صفحة حماية قصيرة.
- تحويل مسار الإنجليزية إلى Hapgood Service Book 1922 (Public Domain) مع استخراج مقاطع الخدمة من النص الأصلي.
- إضافة مسار يوناني من Ευχολόγιον τὸ Μέγα، البندقية 1860، مستودع جامعة يوانينا، بترخيص CC BY 4.0، مع استخراج نص PDF بواسطة pdftotext.
- إضافة fallback آمن على بطاقة المصدر عند تعذر استخراج مصدر مفتوح؛ لا يُعلَن النص كاملاً إذا لم يُستخرج فعليًا.
- منع hard-code لحق إعادة التوزيع: metadata الآن يأخذ permission_confirmed وrights_basis من manifest.
- لا ترجمة آلية، ولا cross-language fallback، ولا proxy خارجي.

## 5.5.1 — إصلاح تنزيل الخدمات الكنسية المحمية

- رفع الإصدار إلى 5.5.1 (50501).
- نقل العماد والذكرانيات الإنجليزية/اليونانية إلى روابط Digital Chant Stand الثابتة حيث تتوفر.
- معالجة HTTP 403 من GOARCH بدون تخفيف شرط اكتمال الخدمات: طلب مباشر بتهيئة متصفح، ثم curl من نفس المصدر الرسمي، ثم Chrome/Chromium headless من نفس المصدر فقط.
- منع أي proxy خارجي أو مرآة غير رسمية أو ترجمة آلية أو fallback بين اللغات.
- إبقاء الخدمة المطلوبة fail-closed إذا فشلت كل طرق الوصول الرسمية؛ لا يتم بناء APK بمحتوى مطلوب ناقص.
...
- عرض أسفار كل عهد وحدها مع بقاء البحث شاملاً للكتاب كله.
- اعتماد أسماء الأناجيل الأربعة كاملة في اللغات الثلاث: العربية والإنجليزية واليونانية.
- نقل نصوص واجهة الكتاب المقدس إلى كتالوجات Android المنفصلة للعربية والإنجليزية واليونانية.
- إزالة عبارة إنجليزية كانت تظهر في سطر المصدر اليوناني، ومنع fallback صامت إلى الإنجليزية عند لغة غير معروفة.
- إضافة اختبارات تمنع خلط لغات واجهة الكتاب أو محتواه وتتحقق من تصنيف أسفار العهد الجديد.

## 5.4.0 — الكتاب المقدس الكامل وإصلاح إعادة الفتح

- تضمين الكتاب المقدس الكامل Offline بالعربية والإنجليزية واليونانية أثناء بناء APK من مصادر eBible المسموح بإعادة توزيعها.
- استخدام ملفات صغيرة لكل سفر لقراءات اليوم والأصحاحات بدل مسح corpus كامل لكل مرجع.
- منع بناء حزمة الأيام التسعة داخل `Application.onCreate()`؛ تظهر الواجهة أولًا ثم يتم أي تحديث لازم في الخلفية.
- إضافة `LocalDailyCacheStore` ذري مع SHA-256 لإعادة فتح التطبيق فورًا على آخر حزمة محلية صحيحة.
- منع إعادة بناء Daily Update عند كل `onResume` عندما بيانات اليوم الحالية جاهزة.
- تأجيل أعمال WorkManager/اختصار المشغل/فحص تحديث APK لما بعد أول إطار مرئي.
- تغيير خلفية نافذة الإطلاق من الأبيض إلى لون التطبيق حتى لا تظهر شاشة بيضاء أثناء التهيئة القصيرة.
- إضافة اختبارات رجعية مخصصة لمشكلة الشاشة البيضاء وإعادة الفتح.

## 5.3.0 — محرك Daily Update محلي مستقل

- نقل تجهيز بيانات اليوم والأيام الثمانية التالية إلى `LocalDailyContentEngine` داخل تطبيق Android.
- جدولة محلية عند 00:03 بتوقيت عمّان مع فحص عند فتح التطبيق ومراقبة تغيّر التاريخ.
- إزالة شرط الشبكة من `DailyUpdateWorker` ومن WorkManager؛ لا توجد إعادة محاولة شبكية أو اعتماد على GitHub Runner.
- حذف `.github/workflows/update.yml` وإبقاء Workflow البناء والنشر الاختياري لنسخ APK/AAB فقط.
- استخدام التقويم المضمّن المحمي من 2026 إلى 2050 ومكتبات الصلوات العربية والإنجليزية واليونانية.
- عرض النص الكتابي فقط عندما يكون موجودًا بدقة في corpus اللغة المضمّن، دون ترجمة آلية أو تخمين.
- رفع الإصدار إلى 5.3.0 / 50300.
...
## 5.6.4 — قداس واحد متصل وحجب OCR السَحَر غير المقروء

- بسّط شاشة القداس إلى مدخل واحد للخدمة الكاملة المعيّنة بحسب اليوم.
- أصلح غياب `service_id` في رزنامة الذهبي الفم، وهو ما كان يمنع تكوين قداس اليوم.
- أضاف تذكار اليوم والصيام إلى رأس القارئ، مع الحفاظ على الرسالة والإنجيل في موضعهما داخل القداس.
- أبقى الصلوات السرّية المعلّمة في المصدر وميّز صلاة الكاهن السرّية وصلاة المؤمن الداخلية.
- أزال السَحَر والساعات والتقدمة وصلوات ما قبل/بعد المناولة من واجهة القداس لتظل خدمات منفصلة.
- حجب نص السَحَر العربي المشوّه الناتج عن OCR من القوائم والبحث حتى إعادة استيراده من مصدر عربي سليم ومراجع.
